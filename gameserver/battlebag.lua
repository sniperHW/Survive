...
function bag:Init(ply,battlebag)
	--for k,v in pairs(battlebag) do
	--	print(v.id,v.count)
	--end
	self.bag = {}
	for k,v in pairs(battlebag) do
		self.bag[v[1] - 4] = {id=v[2],count=v[3],tmpcount = 0}
	end
	for i=1,6 do
		if not self.bag[i] then
			self.bag[i] = {id=0,count=0,tmpcount = 0}
		end
	end
	self.owner = ply
	self.flag = {}
...
function bag:AddItem(id,count)
	...
	for i=1,6 do
		...
		if not pos and self.bag[i].id  == 0 then
			pos = i
		end
	end
	if not pos then
	...
	--print("bag:AddItem",pos)
	if item.count + item.tmpcount >= 5 then
		return false
	end
	if item.count + item.tmpcount + count > 5 then
		local c = 5 - item.tmpcount - item.count
		item.tmpcount = item.tmpcount + c
	else
		item.tmpcount = item.tmpcount + count
	end
	self.flag[pos] = true
	--print("bag:AddItem Success")
	return true
end

function bag:RemItem(id,count)
	local pos
	for i=1,6 do
		if self.bag[i].id == id then
			pos = i
	...
	if pos then
		local item = self.bag[pos]
		if item.count + item.tmpcount < count then
			return false
		end
		if item.tmpcount < count then
			count = count - item.tmpcount
			item.tmpcount = 0
			item.count = item.count - count
		else
			item.tmpcount = item.tmpcount - count
		end
		if item.tmpcount + item.count == 0 then
			item.id = 0
		end
		self.flag[pos] = true
		return true
...
function bag:NotifyUpdate()
	...
	wpk:Write_uint16(NetCmd.CMD_SC_BAGUPDATE)
	...
	for k,v in pairs(self.flag) do
		...
		wpk:Write_uint16(item.count + item.tmpcount)
		c = c + 1
	end
...
function bag:on_entermap(wpk)
	wpk:Write_uint8(6)
	for i = 1,6 do
		local item = self.bag[i]
		wpk:Write_uint8(i+4)
		--print("bag:on_entermap",i+4,item.id)
		wpk:Write_uint16(item.id)
		wpk:Write_uint16(item.count + item.tmpcount)
	end
end
